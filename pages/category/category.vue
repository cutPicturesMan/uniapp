<template>
	...
	<!-- 	<view class="statusBar" :style="{ height: statusBarHeight + 'px' }"></view>
		<view class="getPosition" :style="{ top: statusBarHeight + 'px' }"><text>分类</text></view> -->
						...
						<view class="category-item column" :id="'list' + c_index" v-for="(c_item, c_index) in catrgoryList" :key="c_item.id">
							...
							<view class="category-content">
								<view class="product-item" v-for="(p_item, p_index) in c_item.childList" :key="p_item.id" @click="toCategory(p_item)">
									...
									<text class="product-title">{{ p_item.name || '商品名称' }}</text>
								</view>
							</view>
						</view>
					</view>
...
export default {
	...
	data() {
		return {
			...
			catrgoryList: [
				{ adimg: '', commodityTypes: [{ typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }] },
				{ adimg: '', commodityTypes: [{ typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }] },
				{ adimg: '', commodityTypes: [{ typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }] },
				{ adimg: '', commodityTypes: [{ typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }] },
				{ adimg: '', commodityTypes: [{ typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }, { typeImg: '', name: '' }] },
	...
	methods: {
		...
		async getData() {
			let params = {};
			this.catrgoryList = await Api.apiCall('get', Api.goods.getGoodsTypes, params);
		...
		toCategory(item) {
			...
			// uni.navigateTo({
			// 	url: './commodity/commodity?typeId=' + item.id
			// });
		},
		getHeightList() {
			let _this = this;
		...
		myscroll(e) {
			...
			let scroll_top = e.detail.scrollTop + 180; //110是banner图的高度
