<template>
	<view class="content">
		...
		<view class="row b-b">
			...
			<text @click="chooseLocation" class="input">{{ addressData.detailAddress }}</text>
			<text class="yticon icon-shouhuodizhi"></text>
		</view>
		<view class="row b-b">
			<text class="tit">门牌号</text>
			<input class="input" type="text" v-model="addressData.region" placeholder="楼号、门牌" placeholder-class="placeholder" />
		</view>
...
export default {
	...
	onLoad(option) {
		...
		if (option.type === 'edit') {
			...
			if (this.addressData.defaultStatus == 1) {
				this.defaule = true;
			} else {
				this.defaule = false;
			}
	...
	methods: {
		...
		async confirm() {
			...
			data = await Api.apiCall('post', Api.goods.addressSave, params);
			if (data) {
