<template>
	<view class="container">
		<!-- #ifdef MP-WEIXIN --> 
		<nav-bar>购物车</nav-bar>
		<!-- <view class="getPosition" :style="[{'margin-top': statusBarHeight + 'px'}]" style="height: 80upx;position: fixed;top: 0;z-index: 99;width: 100%">
			<text>购物车</text>
		</view>
...
export default {
	...
	methods: {
		...
		async loadData() {
			let params = {};
			let list = await Api.apiCall('post', Api.order.cartList, params);
		...
		numberChange(data) {
			...
			data = Api.apiCall('post', Api.order.updateQuantity, params);
			this.calcTotal();
		},
		//删除
