...
export default {
	...
	async onLoad(ops) {
		...
		if (id) {
			...
			let data = await Api.apiCall('get', Api.member.storeDetail, params);
			this.logining = false;
	...
	methods: {
		...
		toFavorite(item) {
			if (!this.hasLogin) {
				...
				let params = { objId: item.id, type: 3, name: item.name, meno1: item.logo, meno2: item.hit, meno3: item.collect };
